...
class _DocumentListPageState extends State<DocumentListPage> {
  ...
  Future<void> _loadDocumentList() async {
    ...
    setState(() {
      _documentList =
          List<Map<String, dynamic>>.from(widget.person['documents'] ?? []);
    });
  }
    ...
    if (result != null) {
      ...
      if (personsListData != null) {
        List<Map<String, dynamic>> personsList =
            List<Map<String, dynamic>>.from(jsonDecode(personsListData));
        int index =
            personsList.indexWhere((p) => p['name'] == widget.person['name']);
        if (index != -1) {
          personsList[index] = widget.person;
          await storage.write(
              key: 'persons_list', value: jsonEncode(personsList));
        }
      }
    }
  }
  ...
  Future<void> _openPdf(String filePath) async {
    ...
    if (result.type != ResultType.done) {
      // Show an error if the PDF failed to open
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('${widget.person['name']} Docs'),
        actions: [
          IconButton(
            icon: Icon(Icons.add),
...
class _AddDocumentPageState extends State<AddDocumentPage> {
  ...
  Future<void> _saveDocument() async {
    if (_selectedFile != null &&
        _docNameController.text.isNotEmpty &&
        _docTypeController.text.isNotEmpty) {
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: _pickPdf,
                child:
                    Text(_selectedFile != null ? 'Change PDF' : 'Select PDF'),
              ),
              if (_selectedFile != null)
